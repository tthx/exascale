...
apply_patches() {
  local src_dir="${1}";
  local new_dir="${dev_home}/ginkgo/${ginkgo_hpx_branch}";
...
build_ginkgo_hpx() {
  ...
  cd "${ginkgo_hpx_src_dir}";
  git pull --recurse-submodules;
  restore "${ginkgo_hpx_src_dir}";
  if [ "${poly}" == "ON" ];
  then
    apply_patches "${ginkgo_hpx_src_dir}";
    dev="ON";
  fi
  ginkgo_hpx_prefix+="${embedded:+/embedded}/${mpi_impl}";
  rm -rf "${ginkgo_hpx_prefix}" "./build";
  mkdir -p "./build";
  cd "./build";
  export CC="${cc}";
  ...
  cmake -G Ninja .. \
    ...
    -DCMAKE_INSTALL_PREFIX="${ginkgo_hpx_prefix}" \
